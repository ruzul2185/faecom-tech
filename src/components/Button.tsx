...
export const PrimaryButton = ({ title, classname = "" }: ButtonProps) => {
  return (
    <div
      className={`relative overflow-hidden font-medium text-white bg-[#ff6041] w-61 h-14.5 flex justify-center items-center cursor-pointer group ${classname}`}
    >
...
export const SecondaryButton = ({ title, classname = "" }: ButtonProps) => {
  return (
    <div
      className={`relative overflow-hidden font-medium text-black bg-white w-61 h-14.5 flex justify-center items-center cursor-pointer group hover:text-white ${classname}`}
    >
      <span className="absolute inset-0 flex justify-center items-center pointer-events-none">
